...
private:
    ...
    typeData BitGetVal(typeData inData, int pos);

    template<typename typeData>
    bool testBit(typeData inData, int pos);

    template<typename typeData>
    typeData getBit(typeData inData, int pos);

    template<typename typeData>
    typeData setBit(typeData inData, int pos);

    template<typename typeData>
    typeData resetBit(typeData inData, int pos);
    ...
    int executeExOP(uint8_t exOp); //extended op codes 0xCB 
    ...
    void INC_8BIT_MEM(uint16_t addr);
    void INC_16BIT(uint16_t& reg);
    //Decrement register
    void DEC_8BIT(uint8_t& reg);
    void DEC_8BIT_MEM(uint16_t addr);
    ...
    void test_Bit(uint8_t reg, int bit);
